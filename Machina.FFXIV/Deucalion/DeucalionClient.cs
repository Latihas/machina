...
public class DeucalionClient : IDisposable {
    ...
    private enum DeucalionOpcode : byte {
        ...
        Debug = 0,
        /// <summary>
        ///     Used to maintain a connection between client and the hook server. The hook will echo a "pong" with the same op when
        ///     it receives a ping
        /// </summary>
        Ping = 1,
        /// <summary>
    ...
    public void Connect(int processId) {
        try {
            ...
            var result = ReadPipe(buffer, _tokenSource.Token).FirstOrDefault();
    ...
    private void ProcessReadLoop(CancellationToken token) {
        try {
            ...
            var lastClientPing = DateTime.Now;
    ...
    private unsafe DeucalionMessage[] ReadPipe(byte[] buffer, CancellationToken token) {
        List<DeucalionMessage> response = [];

        // read all available data into the supplied buffer
        var readerTask = _clientStream.ReadAsync(buffer, 0, buffer.Length, token);
        ...
        fixed (byte* ptr = _streamBuffer) {
            ...
            while (index < _streamBufferIndex) {
                ...
                DeucalionMessage newMessage = new() {
                    header = *messagePtr,
                    data = messagePtr->Length > sizeof(DeucalionHeader) ? new byte[messagePtr->Length - sizeof(DeucalionHeader)] : [],
    ...
    private unsafe void WritePipe(DeucalionMessage message, CancellationToken token) {
        var buffer = new byte[sizeof(DeucalionHeader) + (message.data?.Length ?? 0)];
        ...
        var writerTask = _clientStream.WriteAsync(buffer, 0, buffer.Length, token);
    ...
    public static unsafe (long, byte[]) ConvertDeucalionFormatToPacketFormat(byte[] message) {
        ...
        var convertedMessage = new byte[message.Length + sizeof(Server_MessageHeader) - sizeof(DeucalionSegment)];

        long epoch;
        fixed (byte* ptr = convertedMessage) {
            var headerPtr = (Server_MessageHeader*)ptr;
            fixed (byte* ptr2 = message) {
                var segmentPtr = (DeucalionSegment*)ptr2;
