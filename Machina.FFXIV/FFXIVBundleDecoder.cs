...
public class FFXIVBundleDecoder {
    ...
    public unsafe void StoreData(byte[] buffer) {
        ...
        while (offset < _allocated) {
            ...
            fixed (byte* ptr = _bundleBuffer) {
                var header = *(Server_BundleHeader*)(ptr + offset);
                ...
                var message = DecompressFFXIVMessage(ref header, _bundleBuffer, offset, out var messageBufferSize);
                ...
                if (messageBufferSize > 0) {
                    ...
                    fixed (byte* msgPtr = message) {
                        for (var i = 0; i < header.message_count; i++) {
                            var message_length = ((ushort*)(msgPtr + message_offset))[0];
    ...
    public Tuple<long, byte[]> GetNextFFXIVMessage() => Messages.Count > 0 ? Messages.Dequeue() : null;
    ...
    private unsafe byte[] DecompressFFXIVMessage(ref Server_BundleHeader header, byte[] buffer, int offset, out int ffxivMessageSize) {
        ...
        switch (header.compression) {
            case CompressionType.None:
                ...
                for (var i = 0; i < ffxivMessageSize / 4; i++) {
                    // todo: use unsafe pointer operations
                    var value = BitConverter.ToUInt32(buffer, offset + i * 4 + sizeof(Server_BundleHeader));
            ...
            case CompressionType.Oodle:
                try {
                    ...
                    Trace.WriteLine("FFXIVBundleDecoder: Oodle Decompression error: " + ex, "DEBUG-MACHINA");
                    return null;
                }
                break;
            default:
    ...
    private int ResetStream(int offset) {
    ...
    private static unsafe int GetNextMagicNumberPos(byte[] buffer, int currentOffset) {
        fixed (byte* ptr = buffer) {
            for (var nextOffset = currentOffset + 1; nextOffset <= buffer.Length - 4; nextOffset++) {
