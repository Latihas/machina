...
public class SigScan : ISigScan {
    protected virtual Dictionary<SignatureType, int[]> Signatures => new() {
        {
            ...
            SignatureType.OodleNetwork1UDP_Train, SignatureStringToByteArray("41 83 fd 01 75 ** 48 8b 0f e8 ** ** ** ** eb ** 48 8b ** ** e8")
        }, {
            SignatureType.OodleNetwork1UDP_Decode, SignatureStringToByteArray("74 ** 49 8b ca e8 ** ** ** ** eb ** 48 8b 49 ** e8")
        }, {
            SignatureType.OodleNetwork1UDP_State_Size, SignatureStringToByteArray("48 8b 7f ** 48 85 f6 75 ** 48 89 ** ** e8")
        }, {
    ...
    public unsafe Dictionary<SignatureType, int> Read(IntPtr library) {
        ...
        for (var i = signatureTypes.Count - 1; i >= 0; i--) {
    ...
    private unsafe int GetFirstSignatureOccurrence(int[] signature, IntPtr Start, int maxLength) {
        ...
        for (var i = 0; i < maxLength - signature.Length; i++) {
            var numMatch = 0;
    ...
    private int GetSignaturefromOffset(IntPtr currentAddress, IntPtr startAddress, int startIndex) {
        IntPtr matchAddress =
            ...
            Marshal.ReadInt32(IntPtr.Add(currentAddress, startIndex));
        ...
        var offset = (int)(matchAddress.ToInt64() - startAddress.ToInt64());
    ...
    protected static int[] SignatureStringToByteArray(string pattern) {
        ...
        var bytePattern = new int[pattern.Length / 2];
        for (var i = 0; i < pattern.Length / 2; i++) {
            var tmpSearch = pattern.Substring(i * 2, 2);
