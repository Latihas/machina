...
public class OodleNative_Ffxiv : IOodleNative {
    ...
    private static unsafe IntPtr AllocAlignedMemory(IntPtr cb, int alignment) {
        ...
        var block = Marshal.AllocHGlobal(checked(cb + sizeof(IntPtr) + (alignment - 1)));

        // Align the pointer
        var aligned = (IntPtr)((long)(block + sizeof(IntPtr) + (alignment - 1)) & ~(alignment - 1));
    ...
    public void Initialize(string path) {
        try {
            ...
            lock (_librarylock) {
                ...
                var myMallocPtr = Marshal.GetFunctionPointerForDelegate(_OodleMalloc);
                var myFreePtr = Marshal.GetFunctionPointerForDelegate(_OodleFree);
    ...
    public void UnInitialize() {
        try {
            lock (_librarylock) {
                ...
                if (_libraryHandle != IntPtr.Zero) {
                    var freed = NativeMethods.FreeLibrary(_libraryHandle);
                    if (!freed)
