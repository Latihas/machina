...
public class RawCaptureSocket : ICaptureSocket {
    ...
    private Socket CreateRawSocket(uint localAddress, uint remoteAddress) {
        Socket socket = new(AddressFamily.InterNetwork, SocketType.Raw, _isWindows ? ProtocolType.IP : ProtocolType.Tcp);
        socket.Bind(new IPEndPoint(localAddress, 0));

        socket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.HeaderIncluded, true);

        if (_isWindows) {
            byte[] trueBytes = [3, 0, 0, 0]; // 3 == RCVALL_IPLEVEL, so it only intercepts the target interface
