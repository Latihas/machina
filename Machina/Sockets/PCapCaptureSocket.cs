...
public class PCapCaptureSocket : ICaptureSocket {
    ...
    private readonly ConcurrentQueue<Tuple<byte[], int>> _pendingBuffers = new();
    ...
    private string BuildSource(string host, int port) {
        ...
        StringBuilder source = new("rpcap://", PCAP_BUF_SIZE);
    ...
    private PcapDevice GetDevice(uint localAddress) {
        var devices = PcapDevice.GetAllDevices(_source, ref _auth);
        PcapDevice device;
    ...
    public void StartCapture(uint localAddress, uint remoteAddress = 0) {
        ...
        var device = GetDevice(localAddress);
        if (device == null)
            return;

        var filterText = "ip and tcp";
        if (remoteAddress > 0)
            filterText += " and host " + new IPAddress(remoteAddress);

        bpf_program filter = new();
    ...
    public CapturedData Receive() {
        if (_pendingBuffers.TryDequeue(out var next))
    ...
    private unsafe void RunCaptureLoop(CancellationToken token) {
        ...
        while (!token.IsCancellationRequested) {
            try {
                ...
                var packetDataPtr = IntPtr.Zero;
                var packetHeaderPtr = IntPtr.Zero;

                var layer2Length = _activeDevice.LinkType == DLT_EN10MB ? 14 : _activeDevice.LinkType == DLT_RAW ? 0 : 4; // 14 for ethernet, 0 for raw IP, 4 for loopback

                // note: buffer returned by pcap_next_ex is static and owned by pcap library, does not need to be freed.
                var status = pcap_next_ex(_activeDevice.Handle, ref packetHeaderPtr, ref packetDataPtr);
                if (status == 0) // 100ms timeout
                    continue;
                if (status == -1) // error
                {
                    var error = Marshal.PtrToStringAnsi(pcap_geterr(_activeDevice.Handle));
                    if (!bExceptionLogged)
