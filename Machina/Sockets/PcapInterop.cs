...
#pragma warning disable CA1707 // Identifiers should not contain underscores
    ...
    /// </summary>
    /// <param name="source">
    ///     a char* buffer that keeps the 'source localtion', according to the new WinPcap syntax. This source
    ///     will be examined looking for adapters (local or remote) (e.g. source can be 'rpcap://' for local adapters or
    ///     'rpcap://host:port' for adapters on a remote host) or pcap files (e.g. source can be 'file://c:/myfolder/'). The
    ///     strings that must be prepended to the 'source' in order to define if we want local/remote adapters or files is
    ///     defined in the new Source Specification Syntax.
    /// </param>
    /// <param name="auth">
    ///     a pointer to a pcap_rmtauth structure. This pointer keeps the information required to authenticate
    ///     the RPCAP connection to the remote host. This parameter is not meaningful in case of a query to the local host: in
    ///     that case it can be NULL.
    /// </param>
    /// <param name="alldevsp">
    ///     a 'struct pcap_if_t' pointer, which will be properly allocated inside this function. When the
    ///     function returns, it is set to point to the first element of the interface list; each element of the list is of
    ///     type 'struct pcap_if_t'
    /// </param>
    /// <param name="errbuff">
    ///     a pointer to a user-allocated buffer (of size PCAP_ERRBUF_SIZE) that will contain the error
    ///     message (in case there is one).
    /// </param>
    /// <returns>
    ///     -1 is returned on failure, in which case errbuf is filled in with an appropriate error message; 0 is returned
    ///     on success.
    /// </returns>
...
#pragma warning restore CA2101 // Specify marshaling for P/Invoke string arguments
    ...
    /// </summary>
    /// <param name="source">
    ///     zero-terminated string containing the source name to open. The source name has to include the
    ///     format prefix according to the new Source Specification Syntax and it cannot be NULL.
    /// </param>
    /// <param name="snaplen">
    ///     length of the packet that has to be retained. For each packet received by the filter, only the
    ///     first 'snaplen' bytes are stored in the buffer and passed to the user application. For instance, snaplen equal to
    ///     100 means that only the first 100 bytes of each packet are stored.
    /// </param>
    /// <param name="flags">
    ///     keeps several flags that can be needed for capturing packets. The allowed flags are defined in the
    ///     pcap_open() flags
    /// </param>
    /// <param name="read_timeout">
    ///     read timeout in milliseconds. The read timeout is used to arrange that the read not
    ///     necessarily return immediately when a packet is seen, but that it waits for some amount of time to allow more
    ///     packets to arrive and to read multiple packets from the OS kernel in one operation. Not all platforms support a
    ///     read timeout; on platforms that don't, the read timeout is ignored.
    /// </param>
    /// <param name="auth">
    ///     a pointer to a 'struct pcap_rmtauth' that keeps the information required to authenticate the user on
    ///     a remote machine. In case this is not a remote capture, this pointer can be set to NULL.
    /// </param>
    /// <param name="errbuff">a pointer to a user-allocated buffer which will contain the error in case this function fails</param>
    /// <returns>
    ///     A pointer to a 'pcap_t' which can be used as a parameter to the following calls (pcap_compile() and so on) and
    ///     that specifies an opened WinPcap session. In case of problems, it returns NULL and the 'errbuf' variable keeps the
    ///     error message.
    /// </returns>
#pragma warning disable CA2101 // Specify marshaling for P/Invoke string arguments
    [DllImport("wpcap.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
#pragma warning restore CA2101 // Specify marshaling for P/Invoke string arguments
    ...
    /// <summary>
    ///     Compile a packet filter, converting an high level filtering expression (see Filtering expression syntax) in a
    ///     program that can be interpreted by the kernel-level filtering engine.
    /// </summary>
    /// <param name="p">A pointer to a 'pcap_t' instance previously returned by pcap_open*</param>
    ...
    /// <param name="netmask">
    ///     netmask specifies the IPv4 netmask of the network on which packets are being captured; it is
    ///     used only when checking for IPv4 broadcast addresses in the filter program. If the netmask of the network on which
    ///     packets are being captured isn't known to the program, or if packets are being captured on the Linux "any"
    ///     pseudo-interface that can capture on more than one network, a value of 0 can be supplied; tests for IPv4 broadcast
    ///     addreses won't be done correctly, but all other tests in the filter program will be OK.
    /// </param>
...
#pragma warning restore CA2101 // Specify marshaling for P/Invoke string arguments
    ...
    /// <returns>
    ///     -1 is returned on failure, in which case pcap_geterr() may be used to display the error text; 0 is returned on
    ///     success.
    /// </returns>
    [DllImport("wpcap.dll", CallingConvention = CallingConvention.Cdecl)]
    ...
    ///     when that BPF program is no longer needed, for example after it has been made the filter program for a pcap
    ...
    ///     0 if the timeout set with pcap_open_live() has elapsed. In this case pkt_header and pkt_data don't point to a valid
    ///     packet
