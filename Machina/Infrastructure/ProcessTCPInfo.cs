...
#pragma warning restore 0649
	...
	/// </summary>
	public string ProcessWindowName { get; set; } = "";

	/// <summary>
	///     Window class of the process to return network connection information about
	/// </summary>
	public string ProcessWindowClass { get; set; } = "";
...
#pragma warning restore CA1711 // Identifiers should not have incorrect suffix
	...
	public static IList<uint> GetProcessIDByWindow(string windowClass, string windowName) {
		List<uint> processIDList = [];
		var hWindow = IntPtr.Zero;

		while ((hWindow = FindWindowEx(IntPtr.Zero, hWindow, windowClass, windowName)) != IntPtr.Zero) {
			_ = GetWindowThreadProcessId(hWindow, out var processID);
			if (processID > 0)
	...
	/// <summary>
	///     This retrieves all current TCPIP connections, filters them based on a process id (specified by either ProcessID,
	///     ProcessWindowName or ProcessWindowClass parameter),
	...
	public void UpdateTCPIPConnections(IList<TCPConnection> connections) {
		...
		try {
			if (ret == 0) {
				...
				for (var i = 0; i <= tcpTableCount - 1; i++) {
					var entry = *(MIB_TCPROW_EX*)tmpPtr;
				...
				for (var i = connections.Count - 1; i >= 0; i--) {
					...
					for (var j = 0; j <= tcpTableCount - 1; j++) {
						var entry = *(MIB_TCPROW_EX*)tmpPtr;
