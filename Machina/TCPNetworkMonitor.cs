...
/// <summary>
///     TCPNetworkMonitor: This class uses either Winsock raw socket or a PCap kernel driver to read local network traffic.
///     A C# task polls the received data and
...
///     MonitorType: Specifies whether it should use a winsock raw socket, or use PCap (requires separate kernel driver
///     installation).  Default is a raw socket.
///     ProcessID: Specifies a single process ID to record traffic from
///     ProcessIDList: Specifies a collection of process IDs to record traffic from, to support collecting data from
...
///     UseRemoteIpFilter: boolean that specifies whether to start data capture as connections are detected within the
///     target process (new behavior), or monitor
///     the primary interface for the process and capture all data sent/received on that interface - and filter it.  The
///     new behavior may cause some data to be lost
///     on connection startup, but significantly reduces the filtering overhead caused by other traffic on the network
///     interface.
...
///     DataReceivedEventHandler: Delegate that is called when data is received and successfully decoded through IP and TCP
///     decoders.  Note that a connection identifier is
///     supplied to distinguish between multiple connections.
///     DataSentEventHandler: Delegate that is called when data is sent and successfully decoded through IP and TCP
///     decoders.  Note that a connection identifier is
///     supplied to distinguish between multiple connections.
/// </summary>
