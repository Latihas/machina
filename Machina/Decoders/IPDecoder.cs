...
public class IPDecoder {
	...
	/// </summary>
	public IList<byte[]> Fragments { get; }
		= new List<byte[]>();

	/// <summary>
	///     local timestamp of last successfully processed fragment
	/// </summary>
	public DateTime LastFragmentTimestamp { get; internal set; }
		= DateTime.MinValue;
	...
	/// </summary>
	public DateTime LastIPFragmentTimestamp { get; internal set; }
		= DateTime.MinValue;
	...
	public unsafe void FilterAndStoreData(byte[] buffer, int size) {
		...
		fixed (byte* ptr = buffer) {
			while (offset < size - sizeof(IPv4Header)) {
				...
				var version = (byte)((ptr + offset)[0] >> 4);
				if (version == 6) {
					...
					var header6 = *(IPv6Header*)(ptr + offset);
				...
				var ip4Header = *(IPv4Header*)(ptr + offset);
	...
	public unsafe byte[] GetNextIPPayload() {
		...
		var nextFragments = Fragments.Count == 1
			? Fragments
			: Fragments.OrderBy(x =>
